...
const {
  ...
  loading: loadingRecipesResponse,
  refresh: recipesRefetch,
} = useFetch<PaginatedApiResponse>(endpoints.recipe.index, {
  method: "post",
  body: {
    status: "active",
  },
...
const filteredData = computed(() => {
  ...
  return recipes.value.filter((e) =>
    e.created_by[0].name.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
});
...
function deletePost(Id: string) {
  if (confirm("Are you sure you want to delete this post?")) {
    recipes.value = recipes.value.filter((v) => v._id !== Id);
          ...
          <div class="col-xl-8 col-xxl-9 col-lg-8">
            ...
            >
              <NuxtLink :to="'/recipe/' + v?.slug">
                ...
                >
                  <div v-for="media in v?.media">
                    <div
                      :class="{
                        'img-container': media?.ref_code == 'recipe_image',
                      }"
                    >
                      <img
                        v-if="media?.ref_code == 'recipe_image'"
                        :src="
                          media?.is_local
              ...
              <div class="card-body d-flex p-0">
                ...
                <SharerCard
                  :title="v.title"
                  :url="pitLib.util.url('recipe/' + v.slug)"
                ></SharerCard>
              </div>
              <Comment class="mt-2" target="recipe" :target_id="v._id">
              </Comment>
            </div>
