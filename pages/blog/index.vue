...
const {
  data: blogsResponse,
  loading: loadingBlogsResponse,
  refresh: blogsRefetch,
} = useFetch<PaginatedApiResponse>(endpoints.blog.index, {
  method: "post",
  body: {
    status: "active",
  },
...
function likeToggle(v: any) {
  const item = blogs.value.find((item: any) => item._id === v._id);
    ...
    if (d.status) {
      pitLib.notify(
        pitLib.auth.get()?.name +
          (d?.data.deletedCount ? +" Liked" : " Unliked"),
        v?.title,
        v?.created_by[0]._id
      );
...
function deletePost(Id: string) {
  if (confirm("Are you sure you want to delete this post?")) {
    blogs.value = blogs.value.filter((v) => v._id !== Id);
          ...
          <div class="col-xl-8 col-xxl-9 col-lg-8">
            ...
            >
              <NuxtLink :to="'/blog/' + v?.slug">
                ...
                >
                  <div v-for="media in v?.media">
                    <div
                      :class="{
                        'img-container': media?.ref_code == 'blog_image',
                      }"
                    >
                      <img
                        v-if="media?.ref_code == 'blog_image'"
                        :src="
                          media?.is_local
              ...
              <div class="card-body d-flex p-0">
                ...
                <SharerCard
                  :title="v.title"
                  :url="pitLib.util.url('blog/' + v.slug)"
                ></SharerCard>
              </div>
              <Comment class="mt-2" target="blog" :target_id="v._id"> </Comment>
            </div>
